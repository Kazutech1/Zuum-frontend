...
import { usePromotePost } from '../../Hooks/search/useAllPost'; // Import the hook above
            ...
            <div className="flex gap-4 mb-6 border-b" style={{ borderColor: styles.border }}>
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-2 pb-3 px-2 transition-colors relative ${activeTab === tab.id ? 'text-white font-medium' : 'text-gray-500'
                    }`}
                >
                  <tab.icon /> {tab.label}
                  {activeTab === tab.id && (
              ...
              {getContentList().length === 0 ? (
                ...
                getContentList().map(item => (
                  <motion.div
                    ...
                    className={`flex items-center p-3 rounded-xl border cursor-pointer transition-all ${selectedContent?.id === item.id ? 'ring-2' : 'hover:border-gray-500'
                      }`}
                    style={{
                  ...
                  >
                    ...
                    <div className="w-16 h-16 rounded-lg bg-black overflow-hidden relative flex-shrink-0">
                      ...
                      {/* Promoted Overlay - Only if NOT selected (to avoid clutter) or always? Let's do always but below selection */}
                      {item.promoted && (
                        <div className="absolute top-0 right-0 bg-[#2D8C72] text-white text-[9px] font-bold px-1.5 py-0.5 rounded-bl-lg z-0">
                          PROMOTED
                        </div>
                      )}
                    </div>
                    ...
                    {item.promoted && (
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-xs px-2 py-1 rounded bg-[#2D8C72]/20 text-[#2D8C72] border border-[#2D8C72]/50 font-medium">
                          Active
                        </span>
                      </div>
                    )}
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24">
              <div className="p-6 rounded-2xl border shadow-lg" style={{ backgroundColor: styles.card, borderColor: styles.border }}>
                ...
                {/* Duration Selection */}
                <div className="mb-6">
                  <label className="text-sm font-medium block mb-3">Duration</label>
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={`py-2 px-1 text-xs sm:text-sm rounded-lg border transition-all ${duration === opt.days
                          ? 'bg-[#2D8C72] text-white border-[#2D8C72]'
                ...
                <AnimatePresence>
                  ...
                  {promoteSuccess && responseData && (
                    /* Only showing basic error here, success is now a popup */
                    <></>
                  )}
                </AnimatePresence>
        ...
        {promoteSuccess && (
          ...
          >
            <motion.div
              ...
              className="bg-[#2d2d2d] border border-[#374151] rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl"
            >
              <div className="w-16 h-16 bg-green-500/20 text-green-500 rounded-full flex items-center justify-center mx-auto mb-4">
                <FaCheck size={32} />
              </div>
              <h3 className="text-2xl font-bold text-white mb-2">Promotion Active!</h3>
              <p className="text-gray-400 mb-6">
                Your campaign for <span className="text-white font-medium">"{selectedContent?.title}"</span> has been successfully started for {duration} days.
              </p>
              <button
                onClick={() => {
                  refetch.all();
                ...
                className="w-full py-3 bg-[#2D8C72] text-white rounded-xl font-bold hover:bg-[#246f5b] transition-colors"
              >
                Done
              </button>
            </motion.div>
